...
(defpartial html-doc [body]
  (let [attrs (utils/rendering-info (utils/page-attributes body))
       ...
       [:div#menu
        ...
        [:div#user-info
         (if user
           ...
           [:div
            [:a#login {:href (utils/login-url)} "Login"]
...
(defn content-page [{:keys [heading heading-note sub-heading main]}]
  (let [flash-message (session/flash-get :message)
    ...
    (list
     ...
     (when flash-message [:div.message [:span#flash-text flash-message]])
     (when flash-error   [:div.message [:span#error-text flash-error]])
